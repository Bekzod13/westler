...
import type { AdminGroup, Paginated } from '../types/admin'
...
export function GroupsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (q) params.q = q
      const { data } = await api.get<Paginated<AdminGroup>>('/admin/groups', { params })
      setRows(data.data)
      setTotal(data.total)
    } catch {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Группы</h1>
        ...
        <button
          type="button"
          disabled={languages.filter((l) => l.isActive).length === 0}
          onClick={() => {
            setEditing(null)
            setModalOpen(true)
          }}
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50 text-gray-600">
                  ...
                  <th className="px-4 py-3 font-medium">Элементы</th>
                  <th className="px-4 py-3 font-medium">Строк переводов</th>
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/80">
                    ...
                    <td className="px-4 py-3 text-gray-600">{r.itemCount ?? 0}</td>
                    <td className="px-4 py-3 text-gray-600">{r.translationCount}</td>
