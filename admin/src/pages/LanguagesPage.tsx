...
export function LanguagesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (q) params.q = q
      const { data } = await api.get<Paginated<Language>>('/admin/languages', { params })
      setRows(data.data)
      setTotal(data.total)
    } catch {
  ...
  async function handleDelete(id: number) {
    if (!window.confirm('Удалить язык? Связанные переводы могут пострадать.')) return
    try {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Языки</h1>
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/80">
                    ...
                    <td className="px-4 py-3 font-mono text-gray-900">{r.code}</td>
                    <td className="px-4 py-3 text-gray-800">{r.name}</td>
                    <td className="px-4 py-3">{r.isDefault ? 'Да' : '—'}</td>
                    <td className="px-4 py-3">{r.isActive ? 'Да' : 'Нет'}</td>
