...
import type { Paginated, Partner } from '../types/admin'

const PAGE_SIZE = 10

function previewLabel(p: Partner): string {
  const t = p.translations
  const row = t.ru ?? t.en ?? Object.values(t)[0]
  return row?.title?.trim() || p.link || '—'
}
...
export function PartnersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (q) params.q = q
      const { data } = await api.get<Paginated<Partner>>('/admin/partners', { params })
      setRows(data.data)
      setTotal(data.total)
    } catch {
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50 text-gray-600">
                  ...
                  <th className="px-4 py-3 font-medium">Текст / ссылка</th>
                ...
                {rows.map((r, i) => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/80">
                    ...
                    <td className="max-w-md px-4 py-3 text-gray-800">
                      <div>{previewLabel(r)}</div>
                      <div className="truncate text-xs text-gray-500">{r.link}</div>
                    </td>
